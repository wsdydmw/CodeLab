...
public class InterruptTest {
    ...
    public static void main(String[] args) {
        Thread t = new Thread(() -> {
            int executeTime = 5;
            while (!Thread.currentThread().interrupted()) {//  while (!Thread.currentThread().interrupted())
                try {
                    System.out.println("2. in run() - "
                            + Thread.currentThread().getName()
                            + " going to sleep 5 second, now isInterrupted() is "
                            + Thread.currentThread().isInterrupted());//false
                    Thread.sleep(5000);

                    System.out.println("7. in run() - "
                            + Thread.currentThread().getName()
                            + "woke up");

                } catch (InterruptedException e) {
                    System.out.println("5. in run() - get InterruptedException， now isInterrupted() is "
                            + Thread.currentThread().isInterrupted());// false，注意并不是true，抛出异常的时候“是否请求中断标志”被重置了
